...
expand_net <- function(
  ...
  cat(
    "   Number of actors     :",
    dplyr::n_distinct(net$actor_handle),
    fill = T
  )
  cat(
    "   Number of follows    :",
    dplyr::n_distinct(net$follows_handle),
    fill = T
  )
  ...
  while (i == 1 || nrow(new_follows) > 0 && i <= max_iterations) {
    ...
    prospects <- net |>
      ...
      dplyr::filter(
        ...
        .data$follows_handle != "handle.invalid"
      ) |>
    ...
    if (floor(threshold) > 0) {
      prospects <- prospects |>
        dplyr::filter(.data$n >= threshold | .data$n == max(.data$n))
    } else {
      prospects <- prospects |>
        dplyr::filter(.data$frac >= threshold | .data$frac == max(.data$frac))
    }
    ...
    cat(
      "   Number of prospects meeting the followers threshold of ",
      threshold,
      ": ",
      length(prospects),
      sep = "",
    ...
    cat(
      "   Number of valid prospects also matching the provided keywords: ",
      nrow(prospect_profiles),
      sep = "",
      fill = T
    )
    ...
    if (nrow(prospect_profiles) > 0) {
      ...
      cat(
        "   Number of new actors with followers to add to network: ",
        dplyr::n_distinct(new_follows$actor_handle),
        sep = "",
        fill = T
      )
...
init_net <- function(key_actors, keywords, token) {
  ...
  if (nrow(profiles) == 0) {
    stop("Failed to initialize net; no follows that matches keywords")
  }
...
create_widget <- function(net, profiles, prop = 1) {
  ...
  nodes <- nodes |>
    dplyr::mutate(
      com_label = forcats::fct_infreq(as.character(community)),
      com_label = forcats::fct_lump(.data$com_label, n = 10)
    ) |>
    dplyr::group_by(.data$com_label) |>
    dplyr::mutate(n = dplyr::n(), com_id = dplyr::cur_group_id()) |>
    dplyr::ungroup() |>
    dplyr::mutate(
      com_label = ifelse(
        .data$com_id == max(.data$com_id),
        "Other",
  ...
  for (comm in unique(profiles$community)) {
    ...
    if (nrow(comm_profiles) > 0) {
      ...
      tokens_clean <- quanteda::tokens_remove(
        tokens_obj,
        pattern = c(quanteda::stopwords("en"), "|")
      )
